#!/usr/bin/env python3
"""
Example 48: SCD FPAs - Air Tractor AT-802 Detection MC Optimization
=====================================================================

Monte Carlo optimization study for detecting an Air Tractor AT-802
agricultural aircraft from a ground-based sensor platform, using only
SCD (Semi Conductor Devices) FPA products.

This is the SCD-focused variant of Example 44. All physics models
are identical; only the FPA candidate pool is restricted to SCD.

Scenario
--------
- Sensor platform:     Ground level (0 m)
- Target:              Air Tractor AT-802 at 500 ft AGL (~152 m)
- Required FOV:        50 deg x 50 deg
- Objective:           Maximize detection range

Usage:
    python examples/48_scd_air_tractor_detection_mc.py [--no-plot]
    python examples/48_scd_air_tractor_detection_mc.py --mc-runs 500
"""
...
# =============================================================================
# Scenario Parameters (identical to Example 44)
# =============================================================================

SENSOR_ALTITUDE_M = 0.0
TARGET_ALTITUDE_FT = 500
TARGET_ALTITUDE_M = TARGET_ALTITUDE_FT * 0.3048
FOV_DEG = 50.0

AT802_HEAD_ON_WIDTH_M = 4.5
AT802_HEAD_ON_HEIGHT_M = 3.2
AT802_TARGET_SIZE_M = 3.85

AT802_MWIR_INTENSITY_W_SR = 15.0
AT802_LWIR_INTENSITY_W_SR = 120.0

T_AMBIENT_K = 293.0
P_AMBIENT_HPA = 1013.25

MWIR_EXTINCTION_SL = 0.15
LWIR_EXTINCTION_SL = 0.25

MWIR_SKY_RADIANCE = 5e-3
LWIR_SKY_RADIANCE = 2e-2

GROUND_CLUTTER_FACTOR = 0.6
...
def sample_atmosphere(rng: np.random.Generator,
    ...
    return AtmosphericConditions(
        visibility_km=vis_km, cn2=cn2, extinction_per_km=total_ext,
        path_radiance_factor=path_rad_factor, clutter_factor=clutter,
    )
...
def find_detection_range(target: TargetSignature, atm: AtmosphericConditions,
    ...
    for _ in range(50):
        ...
        e_signal = compute_signal_irradiance(target, r_mid_m, tau_atm, optics.transmission)
        ...
        snr = compute_snr(
            ...
            background_radiance=bg_rad, clutter_factor=atm.clutter_factor,
            n_frames=n_frames, tdi_stages=tdi,
        )
...
def build_candidates(fov_deg: float) -> List[SensorCandidate]:
    ...
    for key, fpa in db.items():
        ...
        for fn in f_numbers:
            ...
            optics = OpticsConfig(focal_length_mm=fl_mm, f_number=fn, transmission=0.80)
            operating = OperatingParams(integration_time_ms=tint_ms, frame_rate_hz=30.0,
                                        num_frames_avg=4)
...
def run_mc_evaluation(candidates: List[SensorCandidate],
                      n_runs: int = 200, seed: int = 48) -> List[MCResult]:
    ...
    for cand in candidates:
        ...
        for i in range(n_runs):
            ...
            det_ranges[i] = find_detection_range(target, atm, cand.assembly, band, SNR_DETECTION)
            rec_ranges[i] = find_detection_range(target, atm, cand.assembly, band, SNR_RECOGNITION)
            idf_ranges[i] = find_detection_range(target, atm, cand.assembly, band, SNR_IDENTIFICATION)
...
def main(args):
    print("=" * 70)
    print("Example 48: SCD FPAs - Air Tractor AT-802 Detection MC Optimization")
    print("=" * 70)
    print()
    ...
    print(f"  AT-802 MWIR intensity: {AT802_MWIR_INTENSITY_W_SR} W/sr")
    print(f"  AT-802 LWIR intensity: {AT802_LWIR_INTENSITY_W_SR} W/sr")
    ...
    mc_results = run_mc_evaluation(candidates, n_runs=args.mc_runs, seed=48)
    ...
    for band, r in sorted(bands_seen.items(), key=lambda x: -x[1].mean_detection_km):
        ...
        print(f"   {band.value:12s}: {c.fpa.name:24s} F/{c.f_number} -> "
              f"Det={r.mean_detection_km:.2f}km")
    print()
    ...
    fig.suptitle('SCD FPAs: Air Tractor AT-802 Detection Optimization\n'
    ...
    ax.axvline(best.mean_detection_km, color='blue', linestyle='--', linewidth=1.5,
               label=f'Mean: {best.mean_detection_km:.2f} km')
    ...
    output_path = os.path.join(output_dir, '48_scd_air_tractor_detection.png')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SCD Air Tractor Detection MC Optimization")
