...
def compute_snr(signal_irradiance: float,
    ...
    For cooled IR detectors:
    - Reference D* ~ 1e11 cm·√Hz/W at NETD=20mK
    - D* scales inversely with NETD
    - NEP = √(Ad × Δf) / D*

    This gives realistic detection ranges that differentiate sensors.
    """
    ...
    # D* scales inversely with NETD (lower NETD = better D*)
    ...
    # Cooled: max ~5e11, HOT: ~1e11, Uncooled: ~1e9
    dstar = min(dstar, 5e11)
    dstar = max(dstar, 1e8)

    # NEP = √(Ad × Δf) / D*
    # Ad in cm², Δf = noise bandwidth ≈ 1/(2τ) for integration-limited
    ...
    # Photon energy ~4e-20 J for MWIR (5μm), ~1.5e-20 J for LWIR (10μm)
