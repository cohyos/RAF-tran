#!/usr/bin/env python3
"""
Example 47: SCD FPAs - Air-to-Air F-16 Detection MC Optimization
=================================================================

Monte Carlo optimization study for detecting a head-on F-16 fighter
aircraft from a sensor platform at 35,000 feet altitude, using only
SCD (Semi Conductor Devices) FPA products.

This is the SCD-focused variant of Example 43. All physics models
are identical; only the FPA candidate pool is restricted to SCD.

SCD FPAs evaluated:
- MWIR: Crane, Blackbird 1920, Sparrow-HD, Mini Blackbird 1280,
         Hercules 1280, Sundra
- LWIR: Pelican-D LW, Bird XGA, Bird 640

Scenario
--------
- Sensor platform:     35,000 ft (10,668 m) altitude
- Target:              F-16 head-on at 35,000 ft (co-altitude)
- Required FOV:        50 deg x 50 deg
- Objective:           Maximize detection range

Usage:
    python examples/47_scd_air_to_air_detection_mc.py [--no-plot]
    python examples/47_scd_air_to_air_detection_mc.py --mc-runs 500
"""
...
F16_HEAD_ON_WIDTH_M = 3.0
F16_HEAD_ON_HEIGHT_M = 2.5
F16_TARGET_SIZE_M = 2.75

F16_MWIR_INTENSITY_W_SR = 25.0
F16_LWIR_INTENSITY_W_SR = 80.0

T_AMBIENT_K = 219.0
P_AMBIENT_HPA = 238.0

MWIR_EXTINCTION_SEA_LEVEL = 0.15
LWIR_EXTINCTION_SEA_LEVEL = 0.25

ALTITUDE_DENSITY_RATIO = math.exp(-ALTITUDE_M / 8500.0)
...
MWIR_SKY_RADIANCE = 1e-4
LWIR_SKY_RADIANCE = 5e-3
...
def run_mc_evaluation(candidates: List[SensorCandidate],
                      n_runs: int = 200, seed: int = 47) -> List[MCResult]:
...
def main(args):
    print("=" * 70)
    print("Example 47: SCD FPAs - Air-to-Air F-16 Detection MC Optimization")
    print("=" * 70)
    print()

    print("Scenario:")
    print(f"  Platform altitude:    {ALTITUDE_FT:,} ft ({ALTITUDE_M:.0f} m)")
    print(f"  Target:               F-16 head-on at {ALTITUDE_FT:,} ft")
    ...
    print(f"  F-16 MWIR intensity:  {F16_MWIR_INTENSITY_W_SR} W/sr")
    print(f"  F-16 LWIR intensity:  {F16_LWIR_INTENSITY_W_SR} W/sr")
    ...
    mc_results = run_mc_evaluation(candidates, n_runs=args.mc_runs, seed=47)
    ...
    for i, r in enumerate(ranking):
        ...
        print(f"   {i+1:4d} | {c.fpa.name:24s} | {c.fpa.detector_type.value:<8s} | "
              ...
              f"{r.mean_detection_km:8.1f} | {r.mean_recognition_km:8.1f} | "
              f"{r.mean_identification_km:8.1f} | "
              f"{r.p10_detection_km:7.1f} | {r.p90_detection_km:7.1f}")
    ...
    fig.suptitle(f'SCD FPAs: Air-to-Air F-16 Detection Optimization\n'
    ...
    ax.axvline(best.mean_detection_km, color='blue', linestyle='--', linewidth=1.5,
               label=f'Mean: {best.mean_detection_km:.1f} km')
    ...
    output_path = os.path.join(output_dir, '47_scd_air_to_air_detection.png')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SCD Air-to-Air F-16 Detection MC Optimization")
